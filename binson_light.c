...
void binson_writer_purge(binson_writer *pw) { _binson_io_purge(&(pw->io)); }
...
void binson_parser_reset(binson_parser *pp)
{
    ...
    pp->block_stack[0] = (bs_item){ .val_type = BINSON_ID_UNKNOWN, .name = (bbuf)BBUF_ZERO };
    pp->cb = NULL;
    pp->cb_param = NULL;
...
bool binson_parser_advance(binson_parser *pp,
                           uint8_t scan_flag,
                           int16_t n_steps,
...
uint8_t _binson_parser_process_one(binson_parser *pp)
{
    ...
    case BINSON_ID_DOUBLE: {
        ...
        if (IS_CLEAN(pp)) {
            ...
            pp->val.int_val = _binson_util_unpack_integer(ptr, 8);
        }
        return BINSON_PARSER_STATE_VAL;
    }
...
binson_tok_size binson_parser_get_string_len(binson_parser *pp) { return pp->val.bbuf_val.bsize; }
...
binson_tok_size binson_parser_get_bytes_len(binson_parser *pp) { return pp->val.bbuf_val.bsize; }
