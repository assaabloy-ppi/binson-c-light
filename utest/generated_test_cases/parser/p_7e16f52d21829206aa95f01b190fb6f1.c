...
int main(void)
{
    uint8_t binson_bytes[50] = "\x40\x14\x01\x41\x14\x01\x42\x14\x01\x42\x10\x01\x14\x01\x43\x46\x00\x00\x00\x00\x00\x00\x08\x40\x14\x01\x44\x45\x14\x01\x45\x18\x02\x01\x14\x14\x01\x46\x40\x41\x14\x01\x47\x42\x10\x00\x14\x00\x43\x41";
    binson_parser p;
    ...
    assert(p.error_flags == BINSON_ID_OK);
    assert(memcmp(rawval->bptr, "\x01\x14", rawval->bsize) == 0);
