...
int main(void)
{
    ...
    rawval = binson_parser_get_string_bbuf(&p);
    assert(p.error_flags == BINSON_ID_OK);
    assert(memcmp(rawval->bptr, "\x14\x14\x14\x14\x14\x14\x14\x14\x14\x14\x14\x14\x13\xef\xbf\xbd\x14\x14\x14\x14\x14\x14", rawval->bsize) == 0);
    assert(binson_parser_next_ensure(&p, BINSON_TYPE_STRING));
    assert(p.error_flags == BINSON_ID_OK);
    rawval = binson_parser_get_string_bbuf(&p);
    assert(p.error_flags == BINSON_ID_OK);
