...
int main(void)
{
    uint8_t binson_bytes[46] = "\x40\x14\x01\x41\x14\x01\x42\x14\x01\x42\x40\x41\x14\x01\x43\x46\x00\x00\x00\x00\x00\x00\x08\x40\x14\x01\x44\x45\x14\x01\x45\x18\x02\x01\x02\x14\x01\x46\x40\x41\x14\x01\x47\x42\x43\x41";
    binson_parser p;
    ...
    assert(binson_parser_field_ensure_with_length(&p, "\x42", 1, BINSON_TYPE_OBJECT));
    assert(p.error_flags == BINSON_ID_OK);
    assert(binson_parser_go_into_object(&p));
    assert(p.error_flags == BINSON_ID_OK);
