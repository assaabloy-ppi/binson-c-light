...
int main(void)
{
    uint8_t binson_bytes[14] = "\x40\x14\x01\x3a\x42\x40\x14\x01\x3a\x42\x43\x41\x43\x41";
    BINSON_PARSER_DEF(p);
    ...
    assert(p.error_flags == BINSON_ID_OK);
    assert(binson_parser_field_ensure_with_length(&p, "\x3a", 1, BINSON_TYPE_ARRAY));
    assert(p.error_flags == BINSON_ID_OK);
