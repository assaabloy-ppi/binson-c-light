...
int main(void)
{
    uint8_t binson_bytes[46] = "\x40\x14\x01\x41\x14\x01\x42\x14\x01\x42\x10\x01\x14\x01\x43\x46\x00\x00\x00\x00\x00\x00\x08\x40\x14\x11\x44\x45\x14\x01\x45\x18\x02\x01\x02\x14\x01\x46\x40\x41\x14\x01\x47\x42\x43\x41";
    binson_parser p;
    ...
    assert(p.error_flags == BINSON_ID_OK);
    assert(dval == 3.0);
    assert(binson_parser_field_ensure_with_length(&p, "\x44\x45\x14\x01\x45\x18\x02\x01\x02\x14\x01\x46\x40\x41\x14\x01\x47", 17, BINSON_TYPE_ARRAY));
    assert(p.error_flags == BINSON_ID_OK);
