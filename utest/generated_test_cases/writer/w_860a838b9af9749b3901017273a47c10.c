...
int main(void)
{
    uint8_t expected[46] = "\x40\x14\x01\x41\x18\x01\x42\x14\x01\x42\x10\x01\x14\x01\x43\x46\x00\x00\x00\x00\x00\x00\x08\x40\x14\x01\x44\x45\x14\x01\x45\x18\x02\x01\x02\x14\x01\x46\x40\x41\x14\x01\x47\x42\x43\x41";
    uint8_t created[46];
    binson_writer w;
    ...
    assert(w.error_flags == BINSON_ID_OK);
    binson_write_bytes(&w, (uint8_t *)"\x42", 1);
    assert(w.error_flags == BINSON_ID_OK);
